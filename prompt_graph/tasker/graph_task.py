    ...
    def run(self):
        ...
        for i in range(1, self.task_num + 1):
            ...
            if self.dataset_name in ['COLLAB', 'IMDB-BINARY', 'REDDIT-BINARY', 'ogbg-ppa']:
                ...
                node_degree_as_features(train_dataset)
                node_degree_as_features(test_dataset)
            ...
            if self.prompt_type == 'GPPT':
                if self.dataset_name in ['COLLAB', 'IMDB-BINARY', 'REDDIT-BINARY', 'ogbg-ppa']:
                    ...
                    node_degree_as_features(processed_dataset)
            ...
            if self.prompt_type == 'None':
                ...
                test_acc, f1, roc, prc = AllInOneGraphEva(test_loader, self.prompt, self.gnn, self.answering, self.output_dim, self.device)
            elif self.prompt_type in ['GPF', 'GPF-plus']:
